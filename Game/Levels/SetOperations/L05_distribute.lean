...
Statement (U : Type)(X Y Z : Set U): X ∩ (Y ∪ Z) = (X ∩ Y) ∪ (X ∩ Z) := by
  ...
  Hint "Now in the goal you see a bunch of intersections and unions with a specific precedence, use `rw` to carefully
   open the definitions of intersection and union."
  ...
  exact hx
  intro a a_1
  Hint "Similar to previous goal, use `rw` and necessary theorems to simplify intersections and unions in both the goal and {a_1}."
  rw[mem_inter_iff, mem_union]
  rw[mem_union, mem_inter_iff, mem_inter_iff] at a_1
  Hint "This is also an exercise in propositional logic similar to previous goal, you can take it from here (Believe in yourself)"
  rw[and_or_left]
  exact a_1
