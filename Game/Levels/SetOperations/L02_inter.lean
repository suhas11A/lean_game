...
Statement (a b c d : ℝ)(h1 : a < c)(h3 : b < d): (Ico a b) ∩ (Ico c d) = Ico c b := by
  ...
  Hint "There is tactic named `clear` that will clear any hypotheses or variable if you think its not usefull anymore."
  ...
  rw[mem_Ico]
  Hint "Recall that exact can be used to close goal that dont directly match one of the hypotheses."
  ...
  rw[mem_Ico] at h
  simp [mem_Ico]
  Hint "Now use theoroms `lt_trans`, `le_of_lt`, etc. to get required inequalities."
  Hint "Look into the tactic `have` as it might be usefull."
