...
Introduction "When there is a biconditional in a hypothesis h: p ↔ q, use `iff_elim h into h1, h2` to invoke
...
Statement (h:x=3 ↔ y=5) : (x=3 → y=5) ∧ (y=5 → x=3) := by
  ...
  Hint "Notice that the goal is of the form p ∧ q. Recall how to invoke the introduction rule for the
  conjunction operator to complete the next step of the proof."
  and_intro
