...
Introduction "
Following along with Example 2.1.16, let's prove a basic set theorem using Lean.
...
(To enter the symbol ⊆ type `\\sub`, and for ∈ type `\\mem` or `\\in`).
...
Statement (U : Type)(A : Set U): A ⊆ A := by
  intro x hx
